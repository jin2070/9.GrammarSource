...
- (SPPageMenu *)pageMenu {
    ...
    if (!_pageMenu) {
        ...
        [_pageMenu setItems:@[@"卷🔍",@"章",@"节",@"路线"] selectedItemIndex:0];
...
- (void)secondViewCreatedNotification:(NSNotification *)notification
{
   ...
   // [self.childViewControllers replacementObjectForKeyedArchi
    ...
    [self addChildViewController:self.thirdView];
    [self addChildViewController:self.fourView];
   // self.childViewControllers[1]=self.secondView;
    //BaseViewController *aa=self.childViewControllers[1];
   //[self pageMenu:(SPPageMenu *)pageMenu itemSelectedFromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex
